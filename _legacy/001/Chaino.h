...
namespace chaino_detail {
    ...
    String strWho = "Chaino device";//string that returned by func0
    ...
    class Params {
        public:
            ...
            inline void add(const byte v)   {_vecStr.push_back(String(v));}
            inline void add(const char v)   {_vecStr.push_back(String(v));}
            inline void add(const short v)  {_vecStr.push_back(String(v));}
            inline void add(const unsigned short v)  {_vecStr.push_back(String(v));}
            inline void add(const int v)    {_vecStr.push_back(String(v));}
            inline void add(const unsigned int v)    {_vecStr.push_back(String(v));}
            inline void add(const long v)   {_vecStr.push_back(String(v));}
            inline void add(const unsigned long v)   {_vecStr.push_back(String(v));}
            ...
            inline unsigned int getUInt(unsigned short p=0) {
            ...
            inline unsigned long getULong(unsigned short p=0) {
            ...
            inline short getUShort(unsigned short p=0) {
            ...
            */
            ...
            ///*
            //2025/08/10: Params객체를 int로 캐스팅하는 연산자들 추가(암시변환 금지)
    ...
    void setWhoString(const String& str){
        strWho = str;
    }
    ...
    //0번 함수(고정)
    void who() {
        String str = strWho + " (I2C address:0x"+String(i2c::address,HEX)+")";
        retParams.add(str);
    }
    ...
    void setWhoStr(const String& strWhoR = strWho) {
        strWho = strWhoR;
    }
    ...
    void init() {
        ...
        funcPtrMap[0]   = who;            //0번 함수는 who()로 고정
...
//===============================================================
// Chaino is a **parent class** of all chaino device classes.
//===============================================================
...
public:
    ...
    static inline void setup() {chaino_detail::init();}
    static inline void loop() {chaino_detail::processLoop();}

    // 함수 등록(user는 1번부터 200번까지 등록 가능)
    ...
    static inline int    getIntArg(int p = 0)     {
        return chaino_detail::argParams.getInt(p);
    }

    static inline float  getFloatArg(float p = 0) {
        return chaino_detail::argParams.getFloat(p);
    }

    static inline long   getLongArg(long p = 0)   {
        return chaino_detail::argParams.getLong(p);
    }

    static inline byte   getByteArg(byte p = 0)   {
        return chaino_detail::argParams.getByte(p);
    }

    static inline short  getShortArg(short p = 0) {
        return chaino_detail::argParams.getShort(p);
    }

    static inline bool   getBoolArg(bool p = 0)   {
        return chaino_detail::argParams.getBool(p);
    }

    static inline double getDoubleArg(double p = 0) {
        return chaino_detail::argParams.getDouble(p);
    }

    static inline String getStringArg(const String& p = chaino_detail::EMPTY_STR) {
        return chaino_detail::argParams.getString(p);
    }
